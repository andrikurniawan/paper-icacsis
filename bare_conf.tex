...
\begin{lstlisting}[firstnumber=1, caption=Implementation of m\_find\_value function, label = mfindvalue]
...
\begin{lstlisting}[caption=Total Donation function pseudocode, label=totaldonation]
predId = []
total = 0
for all result where paged.id = id and paged.page = page
  for id, pred_id in result
    predId.push(pred_id) if predId.indexOf(pred_id) == -1
    for amountholder in r.s(m.rsc(pred_id).title).lower
      total += amountholder.amount if amountholder.amount
    end for
  end for
end for
\end{lstlisting}
