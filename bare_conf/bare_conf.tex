...
% in the abstract
\begin{abstract}
Software Product Line (SPL) promises to accelerate the development process with higher quality of product and low budget of production. The essential key of SPL is feature diagram which describes the relation between features for the domain and capture the commonalities and variabilities. The approach gains numerous attention in research and industry area. However, the implementation of SPL approach for information system development is still inadequate. Some works explain the inclusion of ontology for SPL. On the other side, the development of executable modelling language has already begun to model the feature diagram, that is ABS. We propose an adaptor to integrate the ABS microservices and ontology-based information system to produce automatic business logic into the system. This study is implemented with BSMI charity organization as a case study. We show that by using the ontology as its based, the system is semantically structured and the business logic required by the system is updated automatically.
